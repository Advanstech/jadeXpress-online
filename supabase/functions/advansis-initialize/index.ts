...
function generateReference(orderNumber: string) {
  ...
  return `ADV-${orderNumber.slice(-6)}-${rand}`;
}
...
/**
 * Order initialization through the Advansis Technologies gateway (GT Bank
 * payment API: Mobile Money, debit & credit cards).
 *
 * MOCK: no money moves. This creates the order as `pending/unpaid` and
 * returns a gateway reference, mirroring the real Advansis flow. Flip
 * `PAYMENT_MODE` to "live-advansis" and swap the mock for the real GT Bank /
 * Advansis charge call here — the request/response shape stays the same.
 */
